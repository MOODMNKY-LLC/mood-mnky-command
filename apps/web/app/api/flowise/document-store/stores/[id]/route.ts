/**
 * GET /api/flowise/document-store/stores/[id]
 * Proxies to Flowise get document store by ID.
 */
...
export async function GET(
  ...
  const res = await flowiseFetch(`document-store/store/${id}`, { method: "GET" }, userApiKey);
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    return NextResponse.json(
      { error: (data as { message?: string })?.message ?? "Failed to get document store" },
      { status: res.status },
    );
  }
