...
interface PersonaProps {
  ...
  /** CSS variable name for theme-aware color (e.g. '--verse-text-rgb'). Value should be "r, g, b". When set, overrides default light/dark black/white. */
  themeColorVariable?: string;
}
...
function getRgbFromTheme(theme: "light" | "dark"): [number, number, number] {
  return theme === "dark" ? [255, 255, 255] : [0, 0, 0];
}

function getRgbFromCssVariable(
  variable: string
...
const PersonaWithModel = memo(
  ({
    ...
    useEffect(() => {
      ...
      const rgb =
        themeColorVariable != null
          ? getRgbFromCssVariable(themeColorVariable) ?? getRgbFromTheme(theme)
          : getRgbFromTheme(theme);
  ...
  ({
    ...
    return (
      <Component
        rive={rive}
        source={source}
        themeColorVariable={themeColorVariable}
      >
