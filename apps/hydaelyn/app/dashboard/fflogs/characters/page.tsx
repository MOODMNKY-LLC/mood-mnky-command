...
import { FflogsCharactersClient } from "./fflogs-characters-client";

export default async function DashboardFflogsCharactersPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/auth/signin?redirect=/dashboard/fflogs/characters");
