...
import { fetchAbilities } from "@/lib/fflogs/client";
import { getCached, setCached, abilitiesCacheKey } from "@/lib/fflogs/cache";
...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = abilitiesCacheKey(page);
  const cached = await getCached<{ data: unknown[] }>(supabase, cacheKey, WORLD_GAME_TTL);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchAbilities(tokenRow.access_token, { page, limit });
    await setCached(supabase, cacheKey, "game_abilities", result, null);
