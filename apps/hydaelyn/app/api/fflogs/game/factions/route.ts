...
import { fetchFactions } from "@/lib/fflogs/client";
import { getCached, setCached, factionsCacheKey } from "@/lib/fflogs/cache";
...
export async function GET() {
  ...
  const cacheKey = factionsCacheKey();
  const cached = await getCached<unknown[]>(supabase, cacheKey, WORLD_GAME_TTL);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchFactions(tokenRow.access_token);
    await setCached(supabase, cacheKey, "game_factions", result, null);
