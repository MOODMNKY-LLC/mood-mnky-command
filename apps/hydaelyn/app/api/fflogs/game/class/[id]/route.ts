...
import { fetchClass } from "@/lib/fflogs/client";
import { getCached, setCached, classCacheKey } from "@/lib/fflogs/cache";
...
export async function GET(
  ...
  const cacheKey = classCacheKey(idNum);
  const cached = await getCached<unknown>(supabase, cacheKey, WORLD_GAME_TTL);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchClass(tokenRow.access_token, idNum);
    if (result) await setCached(supabase, cacheKey, "game_class", result, null);
