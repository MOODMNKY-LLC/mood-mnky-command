...
import { fetchExpansions } from "@/lib/fflogs/client";
import { getCached, setCached, expansionsCacheKey } from "@/lib/fflogs/cache";
...
export async function GET() {
  ...
  const cacheKey = expansionsCacheKey();
  const cached = await getCached<unknown[]>(supabase, cacheKey, WORLD_GAME_TTL);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchExpansions(tokenRow.access_token);
    await setCached(supabase, cacheKey, "world_expansions", result, null);
