...
import { fetchEncounter } from "@/lib/fflogs/client";
import { getCached, setCached, encounterCacheKey } from "@/lib/fflogs/cache";
...
export async function GET(
  ...
  const cacheKey = encounterCacheKey(idNum);
  const cached = await getCached<unknown>(supabase, cacheKey, WORLD_GAME_TTL);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchEncounter(tokenRow.access_token, idNum);
    if (result) await setCached(supabase, cacheKey, "world_encounter", result, null);
