...
import { fetchEncounterCharacterRankings } from "@/lib/fflogs/client";
import { getCached, setCached, encounterCharacterRankingsCacheKey } from "@/lib/fflogs/cache";
...
export async function GET(
  ...
  const options: { page?: number; serverRegion?: string; serverSlug?: string; metric?: string } = {};
  ...
  const cacheKey = encounterCharacterRankingsCacheKey(encounterId, options as Record<string, string | number | undefined>);
  const cached = await getCached<unknown>(supabase, cacheKey);
  if (cached) return NextResponse.json(cached);

  try {
    const result = await fetchEncounterCharacterRankings(tokenRow.access_token, encounterId, options);
    if (result != null) await setCached(supabase, cacheKey, "encounter_character_rankings", result, null);
