...
- **Push (Supabase → Notion):** When you upload a cover or panel image in MNKY LABZ (or later, when you edit issue/collection metadata in MNKY LABZ), we push the new values to the corresponding Notion page when the row has a `notion_id`.
...
- **Metadata-only push API:** `POST /api/notion/sync/manga/push` with body `{ type: "issue" | "collection", id: "<uuid>" }` (Supabase row id). Loads the row, builds the Notion property payload, and PATCHes the linked Notion page. Use to re-push after a manual DB edit or to sync metadata without re-saving in MNKY LABZ. Requires manga admin auth (MOODMNKY_API_KEY or admin session).

## Conflict strategy

- **Last write wins** per field. MNKY LABZ uploads and edits are the source of truth for URLs and metadata edited in MNKY LABZ; “Sync from Notion” remains the source when pulling from Notion.
- **Push only immediately after an MNKY LABZ action** (e.g. right after upload or save), not on a timer. This avoids overwriting Notion with stale Supabase data.
