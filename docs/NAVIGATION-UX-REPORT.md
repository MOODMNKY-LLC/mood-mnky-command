...
**Single canonical domain:** The app uses **www.moodmnky.com** as the single canonical domain. Routing is path-based: `/main` (marketing), `/dojo` and `/verse` (storefront and member hub; `/verse` redirects to `/dojo`), `/platform` and `/labz` (back office; `/labz` redirects to `/platform`). The proxy ([apps/web/proxy.ts](apps/web/proxy.ts)) redirects **mnky-command.moodmnky.com** and **mnky-verse.moodmnky.com** to www with path mapping (command root → `/platform`, verse root → `/dojo`). Root `/` on www rewrites to `/main`. **Domain stickiness:** The app uses relative links (`/main`, `/dojo`, `/platform`, etc.), so users stay on the same origin; the Lab link from Main is `/platform` (no cross-domain).
