...
**Three domains and routing:** Host-based routing is handled by [apps/web/proxy.ts](apps/web/proxy.ts) (Next.js 16 proxy). **www.moodmnky.com** and **moodmnky.com** rewrite `/` to `/main`; other paths (e.g. `/dojo`, `/api`, `/auth`) are left unchanged. **mnky-verse.moodmnky.com** rewrites **only** the root path `/` to `/verse` (then next.config redirects `/verse` → `/dojo`); paths like `/dojo`, `/main`, `/api`, `/auth` are left unchanged so navigation and APIs work (no /verse/dojo → /dojo/dojo 404). **mnky-command.moodmnky.com** has no rewrite (root is dashboard/Dojo). All three domains point to the same deployment; the proxy uses the `Host` header to apply the correct rewrite. **Domain stickiness:** The app uses relative links (`/main`, `/dojo`, etc.), so users stay on the domain they started on (www, verse, or command) when navigating; no absolute URLs with a specific host are built for in-app links.
