...
| **Shopify** | `SHOPIFY_STORE_DOMAIN`, `SHOPIFY_ADMIN_API_TOKEN` | Product and collection counts, MNKY LABZ pages count, recent product activity; Shopify Status card and MNKY LABZ Pages link. |
...
- **Section order**: Stats, optional MNKY LABZ hub card, Activity Feed, Quick Actions, Shopify Status, Notion Status. The dashboard page reads `dashboardConfig.sectionOrder` and renders sections in that order.
- **showLabzHubCard**: When `true`, the MNKY LABZ hub card is shown (sync + MNKY LABZ Pages links).
- **showConnectAlert**: When `true`, the “Connect Notion and Shopify” alert is shown when both integrations are disconnected.
- **showLabzPagesCountInStats**: When `true`, the MNKY LABZ hub card shows the MNKY LABZ pages count from the store.
