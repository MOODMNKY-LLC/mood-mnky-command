...
const MT_CONFIG_ERROR =
  "Missing Multi-Tenant Supabase config. Set NEXT_PUBLIC_SUPABASE_MT_URL and SUPABASE_MT_SERVICE_ROLE_KEY. See docs/ENV-MULTITENANT-SUPABASE.md.";
 ...
 */
export function createMTAdminClient(): SupabaseClient {
 ...
 */
export function getMTSupabaseClient(_context: { tenantId: string }): SupabaseClient {
 ...
 */
export function getMTClientWithContext(context: TenantContext): SupabaseClient {
...
export function getMTConfigMissing(): string | null {
  const url = process.env.NEXT_PUBLIC_SUPABASE_MT_URL;
  const key = process.env.SUPABASE_MT_SERVICE_ROLE_KEY;
