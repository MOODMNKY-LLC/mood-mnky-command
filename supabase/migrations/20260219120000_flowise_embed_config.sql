...
create table public.flowise_embed_config (
...
create index flowise_embed_config_scope_idx on public.flowise_embed_config (scope);
...
-- anon: select only (embed on public pages needs to fetch config)
create policy "flowise_embed_config_select_anon"
  on public.flowise_embed_config for select to anon
  using (true);

-- authenticated: select (same as anon for now)
