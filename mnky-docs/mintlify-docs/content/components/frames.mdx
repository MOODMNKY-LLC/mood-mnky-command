---
title: "Frames"
description: "Use the Frame component to wrap images or other components in a container."
icon: "frame"
---

Frames are very helpful if you want to center an image.

<Frame>
  <img src="https://mintlify-assets.b-cdn.net/yellowstone.jpeg" alt="Yellowstone National Park landscape" />
</Frame>

## Captions

You can add additional context to an image using the optional `caption` prop.

<Frame caption="Yosemite National Park is visited by over 3.5 million people every year">
  <img src="https://mintlify-assets.b-cdn.net/yosemite.jpg" alt="Yosemite National Park landscape showing mountains and valley" />
</Frame>

## Props

<ResponseField name="caption" type="string">
  Optional caption text to show centered under your component.
</ResponseField>

<RequestExample>

```jsx Frame
<Frame>
  <img src="/path/image.jpg" alt="Example image in frame component" />
</Frame>
```

```jsx Frame with Captions
<Frame caption="Caption Text">
  <img src="/path/image.jpg" alt="Example image in frame component" />
</Frame>
```

</RequestExample>
