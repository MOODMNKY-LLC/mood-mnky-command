...
- **`https://request.moodmnky.com`** → Traefik → **Jellyseerr** (`10.1.0.120:5055` HTTP).

Repo template: **`infra/mnky-media-stack/traefik-dynamic/moodmnky-media.example.yml`**. A path like **`/request` on the media hostname** is **not** supported on the stock Jellyseerr image (Next.js **`/_next`** assets at root); use the **`request`** subdomain unless you maintain a **custom build** with **`basePath`**.

**pfSense:** Unbound overrides **`media`** and **`request`** under **`moodmnky.com` → `10.0.0.25`**. **MOOD-MNKY:** allow **TCP 8096** and **TCP 5055** from **`10.0.0.25`** to the LXC.

**LXC:** `configure-jellyfin-reverse-proxy.py --restart`; **`.env.secrets`:** `JELLYFIN_URL`, `JELLYSEERR_PUBLIC_URL` (default `https://request.moodmnky.com` in bootstrap); **`bootstrap-media-integrations.py`**.
