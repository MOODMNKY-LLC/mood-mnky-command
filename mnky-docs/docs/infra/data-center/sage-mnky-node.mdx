---
title: SAGE-MNKY Node Deep Dive
description: Detailed hardware, storage, and network profile of the SAGE-MNKY node (formerly STUD-MNKY) in the MOOD MNKY Proxmox cluster.
---

## Overview

`SAGE-MNKY` (formerly STUD-MNKY) is a compute node with its own dedicated data pool (`STUD-zfs`). It is well-suited for isolated stacks, experiments, and workloads that benefit from a separate ZFS pool from the main CODE-MNKY environment.

This deep dive is based on the hardware snapshot collected at:

```bash
/root/hardware-snapshots/SAGE-MNKY/<timestamp>/
```

(Historical snapshots may exist under `STUD-MNKY` before the node was renamed.)
