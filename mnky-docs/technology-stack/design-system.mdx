...
This document describes the color palette, design tokens, and design architecture for the MOOD MNKY ecosystem: **MOOD MNKY** (main site), **MNKY LABZ** (dashboard), and **MNKY DOJO** (storefront and member hub).

<Info>
**Source of truth:** The app implementation is canonical. Token values and theme behavior are defined in `apps/web/app/globals.css` and section CSS under `apps/web/app/`. The repo doc `docs/DESIGN-SYSTEM.md` is the single design-system reference; this page mirrors it for the docs site.
</Info>
...
**Palette** = Main | Dojo (which look). **Mode** = light | dark (next-themes). They are independent: four combinations (Main+light, Main+dark, Dojo+light, Dojo+dark). In code and docs, use **palette** when you mean Main vs Dojo, and **mode** when you mean light vs dark, so the two are not mixed up.

## Palette themes and mode (data-theme + next-themes)

The app uses **two palettes**, each with **light and dark mode**:

1. **Main** — Black/white/grayscale only, glassmorphic. Used for the main site and MNKY LABZ.
2. **Dojo** — The Dojo theme: slate light (`#f1f5f9`, `#0f172a`, `#475569`), Dojo dark (`#181619`, `#c8c4c4`, `#94a3b8`). Applied to the MNKY DOJO storefront and auth; one palette, one product name.
...
- **Dark:** #181619 background, #c8c4c4 text, #94a3b8 muted/primary. This is the Dojo theme applied to the MNKY DOJO storefront and auth.
...
| **MNKY LABZ** | Root tokens, shadcn | `app-sidebar`, dashboard layout |
