...
3. **Multi-tenant context:** Default tenant is MOOD MNKY LLC (slug `mood-mnky`). Tenant-scoped data (brand copy, design tokens, content) lives in the MT Supabase project (`@mnky/mt-supabase`, `getDefaultTenantSlug`, `getDefaultTenantBrandCopy`). Resolve tenant from slug or validated profile; see `docs/ENV-MULTITENANT-SUPABASE.md` and `docs/MULTITENANT-SCOPE-REGISTER.md`.
4. **Boundaries:** For theme/Liquid and LABZ-page creation, involve **shopify** or **labz** as needed. For storefront/embed work, involve **verse-storefront**. You own implementation patterns, commands, and DevOps flow.
